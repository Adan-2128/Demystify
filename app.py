...
@app.route('/api/generate_clauses', methods=['POST'])
@login_required
def generate_clauses():
    try:
        ...
        perspective_map = {
            'neutral': 'balanced and fair for both parties',
            'tenant': 'more protective/favorable for the tenant',
            'landlord': 'more protective/favorable for the landlord'
        }

        topic = custom_topic if category == 'custom' else category.replace('_', ' ').title()

        system_prompt = f"""You are an expert Indian rental agreement clause drafter.
You write clear, modern, legally-sounding clauses suitable for Karnataka/Maharashtra/Delhi style agreements.
Use professional language. Include reasonable numbers where applicable.
Perspective: {perspective_map[perspective]}

Generate 3–5 different clause variants for the topic: **{topic}**
...
{{
  ...
  "clauses": [
    {{
      ...
      "notes": "Brief explanation why this version is good / when to use it"
    }},
    ...
  ]
}}
